...
function Contacts(props) {
  ...
  let contactList = filteredContacts().map((contact) => {
    let avatar = contact.avatar_url.length > 0 ?
      (<img src={ contact.avatar_url } className={ styles.avatar } alt={ contact.full_name } />) :
    ...
    <div className={ styles.contacts }>
      <div className={ styles.container }>
        <header>
          ...
          <CloseIcon onClick={ () => { props.handleSend(false) } } />
        </header>
        <div className={ styles.search }>
          <SearchIcon />
          <input name="text" placeholder={ i18n.t('asset.contacts.search_placeholder') } value={ text } onChange={(e) => setText(e.target.value)} />
        </div>
        <main>
          <ul>
            { contactList }
