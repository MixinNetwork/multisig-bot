...
export const ApiGetMultisigsOutputs = (ids, threshold, state, offset) => {
  ...
  offset = offset || ''
  return client.requestByToken('GET', `/multisigs/outputs?members=${hash}&threshold=${threshold}&state=${state}&offset=${offset}`, '', storage.getToken())
}
...
export const ApiGetConversation = (id) => {
  ...
  return client.requestByToken('GET', `/conversations/${id}`, '', storage.getToken())
}
...
export const ApiPostAuthenticate = (code) => {
  ...
  return client.requestByToken('POST', '/oauth/token', params, '').then((resp) => {
    if (resp.error) {
      return resp
    }
