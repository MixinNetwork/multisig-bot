...
import { ApiGetChains, ApiGetMultisigAssets, ApiGetAssets } from "../api";
  ...
  async loadFullData() {
    ...
    assets = assets
      .filter((asset) => {
        ...
        return (
          multisigAssetSet[asset.asset_id] ||
          asset.chain_id === "43d61dcd-e413-450d-80b8-101d5e903357"
        );
      })
      .sort((a, b) => {
    ...
    this.setState({
      assets: assets,
      selectedAssets: storage.getSelectedAssets(),
    });
  }
    ...
    let assets = this.filteredAssets().map((asset) => {
      return (
        <li
          ...
          key={asset.asset_id}
          onClick={() => this.toggleSelect(asset.asset_id)}
        >
          <div className={styles.state}>
            {state.selectedAssets[asset.asset_id] === 0 ? (
              <SelectedIcon />
            ) : (
              <SelectIcon />
            )}
    ...
    return (
      <div className={styles.modal}>
        <div className={styles.container}>
          <header>
            <div className={styles.title}>{i18n.t("home.modal.title")}</div>
          ...
          <div className={styles.search}>
            ...
            <input
              name="text"
              placeholder={i18n.t("home.modal.search_placeholder")}
              value={state.text}
